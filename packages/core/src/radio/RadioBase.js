...
import { Circle } from '@hs/icons'
import useRipple from '../hooks/useRipple'

const useStyles = makeStyles({ name: 'RadioBase' })((theme, props, classes) => ({
	root: {
		display: 'inline-flex',
		position: 'relative',
		height: theme.size(2),
...
const RadioBase = forwardRef(
	(
		...
		const ripple = useRipple({ center: true })
		...
		return (
			<div className={className} {...ripple} {...otherProps}>
				<Input
					...
					{...inputOtherProps}
				/>
				<Circle size={1} color={color} className={classes.check} />
			</div>
		)
	}
)
